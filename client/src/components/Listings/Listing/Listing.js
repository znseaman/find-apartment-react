...
export default function Listing(props) {
  ...
  return (
    <article
      ...
      style={{
        boxShadow: isPopup ? 'none' : '0 2px 8px rgba(0, 0, 0, 0.26)',
        backgroundColor: favorite ? '#ffc107' : 'white',
      }}
    >
      <header
        style={{
        ...
        >
          {amenities.split(', ').map((amenity, i) => {
            if (amenity.match('dogs')) {
              ...
              return (
                <Emoji key={i} label="wheelchair accessible" symbol="♿️" />
              )
            } else if (amenity.match('w/d in unit')) {
              return (
                <Emoji key={i} label="washer and dryer in unit" symbol="👖" />
              )
            } else if (amenity.match('EV charging')) {
              return (
                <Emoji key={i} label="electric vehicle charging" symbol="🔋" />
              )
            }
