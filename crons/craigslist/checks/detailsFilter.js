...
function detailsFilter(details) {
  return new Promise(async (resolve, reject) => {
    ...
    if (!hasCoords) {
      return reject('detailsFilter - Ignored details lacking easily parsable coordinates');
    }
    ...
    if (noPets.test(description)) {
      return reject('detailsFilter - Ignored details as they are not pet-friendly');
    }
    ...
    if (!poly) {
      return reject('detailsFilter - Ignored details as they are not within the City of Vancouver');
    }
